...
class DeepSeekEmbedder:
    """
    DeepSeek embedding provider.
    Supports DeepSeek's embedding models.
    """
    
    def __init__(self, config: EmbeddingConfig):
        """Initialize DeepSeek embedder."""
        self.config = config
        self.api_key = config.api_key or config.additional_params.get("deepseek_api_key")
        self.base_url = config.additional_params.get("base_url", "https://api.deepseek.com/v1")
        self.model = config.model_name or "deepseek-embedding"
        self._client = None
        self._initialized = False
        
        if not self.api_key:
            raise EmbeddingError("DeepSeek API key is required")
        
        logger.info(f"DeepSeekEmbedder initialized with model: {self.model}")
    ...
    async def initialize(self) -> None:
        ...
        try:
            ...
            logger.info("DeepSeek embedder initialized successfully")
            
        except Exception as e:
            logger.error(f"Failed to initialize DeepSeek embedder: {e}")
            raise EmbeddingError(f"DeepSeek initialization failed: {e}")
    ...
    async def _test_connection(self) -> None:
        ...
        try:
            ...
            logger.info(f"DeepSeek API test successful, dimensions: {self.config.dimensions}")
    ...
    async def embed_batch(self, texts: List[str]) -> List[List[float]]:
        ...
        try:
            ...
            batch_size = min(self.config.batch_size, 50)  # Conservative batch size
            all_embeddings = []
            ...
            logger.info(f"Generated embeddings for {len(texts)} texts using DeepSeek")
    ...
    async def close(self) -> None:
        ...
        self._initialized = False
        logger.info("DeepSeek embedder closed")
