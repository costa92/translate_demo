"""DeepSeek LLM provider for answer generation."""
import logging
import httpx
import json
...
class DeepSeekGenerator:
    ...
    def __init__(self, config: GenerationConfig):
        ...
        if not self.api_key:
            raise GenerationError("DeepSeek API key is required")
        
        logger.info(f"DeepSeekGenerator initialized with model: {self.model}")
    ...
    async def initialize(self) -> None:
        ...
        try:
            ...
            logger.info("DeepSeek generator initialized successfully")
        except Exception as e:
            logger.error(f"Failed to initialize DeepSeek generator: {e}")
            raise GenerationError(f"DeepSeek initialization failed: {e}")
...
4. You may quote specific content from the context"""
            ...
            if response.status_code != 200:
                raise GenerationError(f"DeepSeek API request failed: {response.status_code} - {response.text}")
            
            result = response.json()
            
            if "choices" not in result or not result["choices"]:
                raise GenerationError("Invalid DeepSeek API response format")
    ...
    async def close(self) -> None:
        ...
        if self._client:
            ...
            self._client = None
        self._initialized = False
        logger.info("DeepSeek generator closed")