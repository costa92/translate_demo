...
class VectorStore:
    ...
    async def initialize(self) -> None:
        ...
        try:
            ...
            if self.config.provider == "memory":
                ...
                self._provider = MemoryVectorStore(provider_config)
    ...
    async def save(self) -> bool:
        """
        Manually save data to disk if the provider supports it.
        ...
        try:
            if hasattr(self._provider, 'save'):
                logger.info("Manually saving vector store data")
                return await self._provider.save()
            else:
                logger.warning(f"Provider {self.config.provider} does not support manual saving")
                return False
        except Exception as e:
    ...
    async def load(self) -> bool:
        """
        Manually load data from disk if the provider supports it.
        ...
        try:
            if hasattr(self._provider, 'load'):
                logger.info("Manually loading vector store data")
                return await self._provider.load()
            else:
                logger.warning(f"Provider {self.config.provider} does not support manual loading")
                return False
        except Exception as e:
